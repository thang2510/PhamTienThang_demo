...
public class BoardController : MonoBehaviour
{
    ...
    public List<NormalItem.eNormalType> pickedTypes = new List<NormalItem.eNormalType>();
    ...
    public void Update()
    {
        ...
        if (Input.GetMouseButtonDown(0))
        {
            ...
            if (hit.collider != null)
            {
                ...
                {
                    NormalItem item = clickedCell.Item as NormalItem;
                    if (item != null)
                    {
                        Debug.Log($"Clicked on cell ({clickedCell.BoardX},{clickedCell.BoardY}) | Type={item.ItemType}");

                        // Lấy vật ra
                        clickedCell.ExplodeItem();
                        StartCoroutine(ShiftDownItemsCoroutine());

                        // Lưu loại vật
                        pickedTypes.Add(item.ItemType);

                        // Kiểm tra 3 loại giống nhau
                        CheckPickedItems();
                    }
                }
            }
    ...
    {
        // Kiểm tra nếu có ít nhất 3 vật được chọn
        if (pickedTypes.Count < 3) return;

        // Đếm loại cuối cùng
        NormalItem.eNormalType lastType = pickedTypes[pickedTypes.Count - 1];
        int count = 0;

        foreach (var t in pickedTypes)
        {
            if (t == lastType)
                count++;
        }
        ...
        if (count >= 3)
        {
            Debug.Log($"🔥 Có 3 item cùng loại ({lastType}) → +1 điểm!");
            pickedTypes.Clear(); // reset danh sách để tính tiếp
        }
    }
